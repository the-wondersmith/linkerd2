...
use linkerd_policy_controller_core::{
    ...
    routes::{GrpcRouteMatch, HttpRouteMatch},
...
fn to_server(srv: &InboundServer, cluster_networks: &[IpNet]) -> proto::Server {
    ...
    let protocol = proto::ProxyProtocol {
        kind: match &srv.protocol {
            ...
            ProxyProtocol::Grpc(routes) => Some(proto::proxy_protocol::Kind::Grpc(
                    proto::proxy_protocol::Grpc {
                        routes: to_grpc_route_list(routes, cluster_networks),
                    },
                )),
...
fn to_grpc_route_list<'r>(
    routes: impl IntoIterator<Item = (&'r InboundRouteRef, &'r InboundRoute<GrpcRouteMatch>)>,
    cluster_networks: &[IpNet],
) -> Vec<proto::GrpcRoute> {
    // Per the Gateway API spec:
    //
...
fn convert_grpc_filter(filter: Filter) -> Option<proto::grpc_route::Filter> {
    ...
    let kind = match filter {
        Filter::ResponseHeaderModifier(_) | Filter::RequestRedirect(_) => None,
        Filter::FailureInjector(f) => Some(Kind::FailureInjector(
            routes::grpc::convert_failure_injector_filter(f),
        )),
        Filter::RequestHeaderModifier(f) => Some(Kind::RequestHeaderModifier(
            routes::convert_request_header_modifier_filter(f),
        )),
